...
class alexadeezer extends eqLogic {
	...
	public static function createNewDevice($deviceName, $deviceSerial) {
		...
		$newDevice = new alexadeezer();
		$newDevice->setName($deviceName);
		$newDevice->setLogicalId($deviceSerial);
		$newDevice->setEqType_name('alexadeezer');
	...
	public function refresh() { 
		...
		log::add('alexadeezer', 'info', 'Refresh du device '.$this->getName().' ('.$devicetype.')');
		...
		if ($_playlists) {
			...
			if (is_object($cmd)) { //Playlists existe on  met à jour la liste des Playlists
				...
				$cmd->save();
				log::add('alexadeezer', 'debug', 'Mise à jour de la liste des Playlists de '.$this->getName());
			}
		}
		...
		if ($Test) {
			try {
				...
				if ((!is_object($cmd)) || $forceUpdate) {
					if (!is_object($cmd)) $cmd = new alexadeezerCmd();
					$cmd->setType($Type);
				...
				log::add('alexadeezer', 'error', __('Erreur pour ', __FILE__) . ' : ' . $exc->getMessage());
			}
		} else {
							//log::add('alexadeezer', 'debug', 'PAS de **'.$LogicalId.'*********************************');
	...
	public function postSave() {
		//log::add('alexadeezer', 'debug', '**********************postSave '.$this->getName().'***********************************');
		...
		log::add('alexadeezer', 'warning', 'Pas de capacité détectée sur '.$this->getName().' , assurez-vous que le démon est OK');
		}
		$this->refresh(); 
		$this->setStatus('forceUpdate', false); //dans tous les cas, on repasse forceUpdate à false
				//log::add('alexadeezer', 'debug', '**********************fin postSave '.$this->getName().'***********************************');
	...
	public function preRemove () {
		...
		$eq=eqLogic::byLogicalId($device_playlist,'alexadeezer');
...
// https://jeedom.github.io/documentation/dev/fr_FR/widget_plugin	
  ...
  public function toHtml($_version = 'dashboard') {
	...
	foreach ($this->getCmd('info') as $cmd) {
		 	//log::add('alexadeezer_widget','debug',$typeWidget.'dans boucle génération Widget');        
...
class alexadeezerCmd extends cmd {
	...
	public function preSave() {
		...
		$actionInfo = alexadeezerCmd::byEqLogicIdCmdName($this->getEqLogic_id(), $this->getName());
		...
		if (($this->getType() == 'action') && ($this->getConfiguration('infoName') != '')) {//Si c'est une action et que Commande info est renseigné
			$actionInfo = alexadeezerCmd::byEqLogicIdCmdName($this->getEqLogic_id(), $this->getConfiguration('infoName'));
			if (!is_object($actionInfo)) {//C'est une commande qui n'existe pas
				$actionInfo = new alexadeezerCmd();
	...
	public function execute($_options = null) {
		...
		log::add('alexadeezer', 'info', 'Request : ' . $request);//Request : http://192.168.0.21:3456/volume?value=50&device=G090LF118173117U
		...
		//log::add('alexadeezer', 'info', 'resultjson:'.json_encode($resultjson));
					// Ici, on va traiter une commande qui n'a pas été executée correctement (erreur type "Connexion Close")
						...
						log::add('alexadeezer', 'debug', '**On traite '.$value.$detail.' Connexion Close** dans la Class');
						sleep(6);
							if (ob_get_length()) {
							ob_end_flush();
							flush();
		...
		if (($this->getType() == 'action') && (is_array($this->getConfiguration('infoNameArray')))) {
			foreach ($this->getConfiguration('infoNameArray') as $LogicalIdCmd) {
				...
				if (is_object($cmd)) { 
					...
					//log::add('alexadeezer', 'info', $LogicalIdCmd.' prévu dans infoNameArray de '.$this->getName().' trouvé ! '.$resultjson[0]['whennextmusicalalarminfo'].' OK !');
				} else {
					log::add('alexadeezer', 'warning', $LogicalIdCmd.' prévu dans infoNameArray de '.$this->getName().' mais non trouvé ! donc ignoré');
				} 
			}
		} 
		elseif (($this->getType() == 'action') && ($this->getConfiguration('infoName') != '')) {
			// Boucle non testée !!
				...
				if (is_object($cmd)) { 
					...
					log::add('alexadeezer', 'warning', $LogicalIdCmd.' prévu dans infoName de '.$this->getName().' mais non trouvé ! donc ignoré');
				} 
		}
		return true;
	}
	...
	log::add('alexadeezer', 'info', '----Command:*'.$command.'* Request:'.json_encode($_options));
		switch ($command) {
			case 'volume':
		...
		//log::add('alexadeezer_debug', 'debug', '----RequestFinale:'.$request);
	...
	private function build_ControledeSliderSelectMessage($_options = array(), $default = "Ceci est un message de test") {
		...
		//log::add('alexadeezer_node', 'info', '---->Request2:'.$request);
		//log::add('alexadeezer_node', 'debug', '---->getName:'.$this->getEqLogic()->getCmd(null, 'volumeinfo')->execCmd());
		...
		array($_options_slider, $_options_select, urlencode(self::decodeTexteAleatoire($_options_message)), $_options_volume), $request);
		//log::add('alexadeezer_node', 'info', '---->RequestFinale:'.$request);
		return $request;
	}	
	...
	private function buildRestartRequest($_options = array()) {
		log::add('alexadeezer_debug', 'debug', '------buildRestartRequest---UTILISE QUAND ???--A simplifier--------------------------------------');
...
//		if ($_version != 'scenario') return parent::getWidgetTemplateCode($_version, $_noCustom);
		...
		if ($command == 'command' && strpos($arguments, '#select#')) 
			return getTemplate('core', 'scenario', 'cmd.command', 'alexadeezer');
