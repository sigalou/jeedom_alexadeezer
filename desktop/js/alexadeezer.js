    ...
    buttons: {
      ...
      success: {
        ...
        callback: function () {
             $.ajax({
              type: "POST",
                  url: "plugins/alexadeezer/core/ajax/alexadeezer.ajax.php",
              data: {
...
$("#bt_addespeasyAction").off('click').on('click', function(event)
{
 ...
 $('#bt_addEvent').off('click').on('click', function () {
    ...
    $('#md_modal').load('index.php?v=d&plugin=alexadeezer&modal=alarm&eqLogic_id=' + $('.eqLogicAttr[data-l1key=id]').value()).dialog('open');
});

$('#bt_media').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Info Media}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexadeezer&modal=media&iddevice='+ $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_req').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Requêteur JSON}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexadeezer&modal=req&iddevice='+ $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_req2').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Requêteur}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexadeezer&modal=req2&iddevice='+ $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_sante').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Liste Players Deezer}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexadeezer&modal=health').dialog('open');
});

$('#bt_reminders').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Rappels/Alarmes}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexadeezer&modal=reminders').dialog('open');
});

$('#bt_history').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Historique}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexadeezer&modal=history').dialog('open');
});

$('#bt_routines').off('click').on('click', function ()
{
  $('#md_modal').dialog({title: "{{Routines}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexadeezer&modal=routines').dialog('open');
});
...
{
	// Ici il faudra faire comme dans desktop/php/alexadeezer.php ligne 268 chercher type et à défaut family
...
jeedom.plugin.getDeamonCookieInfo = function(_params)
{
    ...
    paramsAJAX.url = 'plugins/alexadeezer/core/ajax/alexadeezer.ajax.php';
    paramsAJAX.data = {
...
jeedom.plugin.VerifiePresenceCookie = function(_params)
{
 ...
 jeedom.plugin.node.action = function (_params) { //Delete reminder
 	...
 	var params = $.extend({}, jeedom.private.default_params, paramsSpecifics, _params || {});
 	var paramsAJAX = jeedom.private.getParamsAJAX(params);
 	paramsAJAX.url = 'plugins/alexadeezer/desktop/php/alexadeezerProxy.php';
 	paramsAJAX.data = {
