...
/*
	DEPLACE DANS 	public static function ScanAmazonAlexa($_logical_id = null, $_exclusion = 0) de core class alexadeezer.class.php
...
foreach($json as $item)
	{
		...
		$alexadeezer = alexadeezer::byLogicalId($serial, 'alexadeezer');
		if (!is_object($alexadeezer)) {
			$alexadeezer = new alexadeezer();
			$alexadeezer->setName($device);
			$alexadeezer->setLogicalId($serial); 
			$alexadeezer->setEqType_name('alexadeezer');
			$alexadeezer->setIsEnable(1);
			$alexadeezer->setIsVisible(1);
		}
		$alexadeezer->setConfiguration('serial',$serial); 
		$alexadeezer->setConfiguration('device',$device);
		$alexadeezer->setConfiguration('type',$type);
		$alexadeezer->setStatus('online',$online);
		$alexadeezer->save();
 }
*/

$eqLogics = alexadeezer::byType('alexadeezer');
?>
